...
w0_true<-rmnorm(n=1,
                mean=rep(0, times=sum(sample_size)),
                varcov=Sigma0_true)
w0_true<-w0_true - 
         mean(w0_true)
...
alpha_true<-rmnorm(n = 1,
                   mean = rep(0, 
                              times = m),
                   varcov = CAR_cov_true)
alpha_true<-(alpha_true - mean(alpha_true))/sd(alpha_true)

mu_true<-1.00

########################################################
#Creating Lagged Covariates and AQS Data
########################################################
L<-11
...
true<-c(sigma2_epsilon_true,
        (beta0_true + w0_true[CMAQ_key_test_set == 1]),
        (beta1_true + w1_true[CMAQ_key_test_set == 1]))
...
est<-c(mean(dlfuse_results$sigma2_epsilon[keep_set]), 
       rowMeans(matrix(dlfuse_results$beta0[keep_set], 
                       ...
                       ncol=length(keep_set)) + dlfuse_results$w0[,keep_set]),
       rowMeans(matrix(dlfuse_results$beta1[keep_set], 
                       nrow=sum(sample_size[test_set]), 
                       ncol=length(keep_set)) + dlfuse_results$w1[,keep_set]))
